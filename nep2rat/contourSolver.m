...
if isfield(opts, 'thres')
    thres = opts.thres;
    if ~(isnumeric(thres) && isscalar(thres))
...
if isfield(opts, 'maxRefine')
    ...
    if ~(isnumeric(maxRefine) && isscalar(maxRefine))
...
if isfield(opts, 'NewtRefs')
    ...
    if ~(isnumeric(NewtRefs) && isscalar(NewtRefs))
...
if isfield(opts, 'maxRecLvl')
    ...
    if ~(isnumeric(maxRecLvl) && isscalar(maxRecLvl))
...
if isfield(opts, 'nF')
    if (isnumeric(opts.nF) && isscalar(opts.nF))&& opts.nF >= 0
        nF = opts.nF;
    else
